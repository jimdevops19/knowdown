...
accurate, and slow (a response band centered past 9 seconds) — and bot *n* is
the strongest — ~90% accurate and fast (centered under 2 seconds) — with every
bot in between interpolated linearly on both axes together. That is a
deliberate choice, not the only valid one: a slow-but-accurate or
...
#: The full span the roster interpolates across. Matches the brief precisely:
#: the weakest bot's guessing probability is 30%, the strongest's is 90%; the
#: weakest bot's response band centers past 9s, the strongest's under 2s.
_MIN_ACCURACY = 0.30
_MAX_ACCURACY = 0.90
_SLOWEST_MEAN_MS = 9_500
_FASTEST_MEAN_MS = 1_500
#: Width of one bot's own response band. Narrow enough that a bot is
#: recognisably fast or slow rather than merely random, wide enough that the
#: same bot does not answer at the exact same millisecond every question.
_RESPONSE_BAND_WIDTH_MS = 1_200
_MIN_RESPONSE_FLOOR_MS = 300
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        for level in range(1, count + 1):
            display_name, accuracy, min_ms, max_ms = _bot_spec(level=level, count=count)
            if options["dry_run"]:
                self.stdout.write(
                    f"{display_name}: accuracy={accuracy:.0%} "
                    f"response={min_ms}-{max_ms}ms"
                )
                continue
            was_created = _upsert_bot(
                display_name=display_name, accuracy=accuracy, min_ms=min_ms, max_ms=max_ms
            )
            created += was_created
...
def _bot_spec(*, level: int, count: int) -> tuple[str, float, int, int]:
    ...
    mean_ms = _SLOWEST_MEAN_MS + fraction * (_FASTEST_MEAN_MS - _SLOWEST_MEAN_MS)
    min_ms = max(_MIN_RESPONSE_FLOOR_MS, round(mean_ms - _RESPONSE_BAND_WIDTH_MS / 2))
    max_ms = round(mean_ms + _RESPONSE_BAND_WIDTH_MS / 2)
    return f"CPU-{level:02d}", round(accuracy, 4), min_ms, max_ms


@transaction.atomic
def _upsert_bot(*, display_name: str, accuracy: float, min_ms: int, max_ms: int) -> bool:
    ...
    BotProfile.objects.update_or_create(
        player=player,
        defaults={"accuracy": accuracy, "min_response_ms": min_ms, "max_response_ms": max_ms},
    )
    return created
