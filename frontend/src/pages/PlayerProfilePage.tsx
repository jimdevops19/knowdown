...
export function PlayerProfilePage() {
  ...
  const { display_name, mascot, rankings, badges } = profile.data
  const isMe = profile.data.id === user?.player_id

  return (
    <div className="flex flex-col gap-6">
      <header className="flex items-center gap-4">
        <Avatar name={display_name} mascot={mascot} size={72} ring={isMe} />
  ...
  return (
    <Card className="flex items-center gap-3 p-4" border="border-gold/25">
      {badge.icon_url ? (
        ...
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-tile bg-gold/15 font-display text-lg font-bold text-gold">
